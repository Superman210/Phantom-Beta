...
(function() {
  ...
  tagsInput.directive("autoComplete", ["$document", "$timeout", "$sce", "$q", "tagsInputConfig", "tiUtil", function($document, $timeout, $sce, $q, tagsInputConfig, tiUtil) {
    function SuggestionList(loadFn, options, events) {
      ...
      self.show = function() {
        if (options.selectFirstMatch && self.items.length) {
          self.select(0);
        }
        else {
          self.selected = null;
  ...
  tagsInput.directive("tiAutocompleteMatch", ["$sce", "tiUtil", function($sce, tiUtil) {
    return {
      ...
      link: function(scope, element, attrs, autoCompleteCtrl) {
        ...
        scope.$getDisplayTextAuto =  function() {
  ...
  tagsInput.run(["$templateCache", function($templateCache) {
    ...
    <div class="tags" ng-class="{focused: hasFocus}">
      <ul class="tag-list" style="display: block;width: 100%;overflow: hidden;">
      ...
      <input class="input" autocomplete="off" 
        ...
        ti-bind-attrs="{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}" ti-autosize>
      </div>
    </div>
    </div>`);
    ...
    `<div class="autocomplete" ng-if="suggestionList.visible">
      <ul class="suggestion-list">
        <li class="suggestion-item" 
            ng-repeat="item in suggestionList.items" 
    ...
    `<span ng-bind-html="$highlight($getDisplayTextAuto())"></span>
    <div style="font-size: 14px;">{{data.owner}}: {{data.desc}}</div>`
    );
  }]);
